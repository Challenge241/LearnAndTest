...
public class CardLevelUp : MonoBehaviour
{
    ...
    public void OnEndDrag(BaseEventData data)
    {
        ...
        {
            //判断物体为植物，并且该植物可升级
            if (c.tag == "Plant"&& c.GetComponent<Plant>().canLevelUp==true)
            {
                c.GetComponent<Plant>().haveLevelUp = true;
                //Debug.LogWarning("haveLevelUp");
                //销毁图片
            ...
            if (c.tag == "Spike" && c.GetComponent<Spikeweed>().canLevelUp == true)
            {
                c.GetComponent<Spikeweed>().haveLevelUp = true;
                //Debug.LogWarning("haveLevelUp");
                //销毁图片
                Destroy(curGameObject);
                //拿到物体预制件
